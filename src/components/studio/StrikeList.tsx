    ...
    <div className="ks-reveal mt-3 grid gap-4 rounded-3xl bg-ks-secondary p-[26px] text-left md:mt-5 md:grid-cols-2 md:gap-12 md:rounded-[28px] md:p-10 lg:p-12">
      ...
      <ul className="flex flex-col">
        {s.items.map((item) => (
          ...
          >
            <span className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full bg-white text-ks-pink md:h-6 md:w-6" aria-hidden="true">
              <Cross size={10} />
            </span>
            <span className="line-through decoration-[rgba(255,46,110,0.55)] decoration-[length:1.5px]">{item}</span>
          </li>
        ))}
      </ul>
    </div>
