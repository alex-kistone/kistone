...
const need: any = {
  ...
  mission_location: "Paris", remote_policy: "on-site",
...
const recruiters: any[] = [
  { ...base, id: "a", first_name: "Sarah", skills: ["Tech", "SRE", "Go"], sectors: ["Fintech"], mobility: ["Paris"], tjm: 520, admin_rating: 5, super_tam: true, remote_preference: "on-site" },
  ...
  { ...base, id: "e", first_name: "FullRem", skills: ["Tech","SRE"], tjm: 480, remote_preference: "full-remote", mobility: ["Paris"], sectors:["Fintech"] },
...
// Un full-remote sur un besoin sur site doit être fortement pénalisé, pas ignoré.
const all = prefilter(need, recruiters, busy);
const fullRemote = all.find(s => s.recruiter.id === "e")!;
const onSite = all.find(s => s.recruiter.id === "a")!;
console.log("pénalité remote appliquée :", fullRemote.breakdown.remote === 0 ? "OK" : "ÉCHEC");
console.log("full-remote classé sous on-site :", fullRemote.score < onSite.score ? "OK" : "ÉCHEC");
